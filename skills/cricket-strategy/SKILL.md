...
**Distinguish the toss contingency from in-game reactive triggers.** Two different things, and some captains want one but not the other:
- **Toss contingency (usually KEEP):** what we do if we lose the toss / are put in against our choice. This is a real pre-game strategic branch — include the toss call plus the if-we-lose-the-toss plan (adjusted target/approach) in both summaries.
- **In-game reactive triggers (DROP if the captain decides live):** lists like "if a bowler goes for 10+ swap him", "if 2 wickets fall by over 5 promote X", "if their batter is set at over 9 bring on Y". When the captain prefers to call these on the day, omit the reactive trigger tree from both summaries; give the committed plan and the toss contingency, and stop. The plan file may keep an internal note without surfacing it.
